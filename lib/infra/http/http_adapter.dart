...
class HttpAdapter implements HttpClient {
  ...
  Future<dynamic> request({@required String url, @required String method, Map body, Map headers}) async {
    ...
    try {
      if (method == 'post') {
        futureResponse = client.post(Uri.tryParse(url), headers: defaultHeaders, body: jsonBody);
      } else if (method == 'get') {
        futureResponse = client.get(Uri.tryParse(url), headers: defaultHeaders);
      } else if (method == 'put') {
        futureResponse = client.put(Uri.tryParse(url), headers: defaultHeaders, body: jsonBody);
      }
